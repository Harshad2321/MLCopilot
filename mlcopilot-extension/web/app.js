...
function renderDetection(issue) {
    ...
    $detection.innerHTML = `
        <div class="detection-alert ${sev}">
            <div class="det-type">${formatAnomaly(issue.anomaly)} <span style="opacity:0.6">(${(issue.confidence * 100).toFixed(0)}%)</span></div>
...
function renderRecommendations(recs) {
    if (!recs.length) {
        $recs.innerHTML = '<div class="placeholder-text">No recommendations</div>';
        return;
    }
...
async function applyLR() {
    ...
    if (isNaN(lr) || lr <= 0) {
        alert("Enter a valid positive learning rate");
        return;
    }
    const res = await fetch("/set_lr", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ lr: lr })
    });
    const data = await res.json();
